...
class OneLayerNetwork:
    def __init__(self, in_features, out_features):
        ...
        self.W = self._init_layer()
    
    def _init_layer(self):
        # +1 to dims for bias trick & Kaiming initialization
        W = np.random.randn(self.in_features + 1, self.out_features) * np.sqrt(2 / (self.in_features + 1))
        W[-1] *= 0
    ...
    def from_model(path):
        ...
        assert isinstance(model, OneLayerNetwork), "init model is not instance of ThreeLayerNetwork class"
